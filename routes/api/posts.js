...
router.put('/like/:id',auth,async(req,res)=>{
    try {
        ...
        if(post.likes.filter(like=>like.user.toString()===res.user.id).length>0){
...
router.delete('/comment/:id/:comment_id',auth, async(req,res)=>{
    try {
        ...
        const removeIndex= post.comments.map(comment => comment.user.toString()).indexOf(res.user.id);
